package cn.edu.jumy.oa.widget.IndexableStickyListView;
...
public class IndexableStickyListView extends FrameLayout implements AdapterView.OnItemClickListener, AbsListView.OnScrollListener {
     ...
     *
     * @param items          继承IndexEntity的List
     * @param headerEntityArrayList IndexListView自定义Header 如添加定位,热门城市等
     * @param <T>            继承IndexEntity
     */
    public <T extends IndexEntity> void bindDatas(final List<T> items, final ArrayList<IndexHeaderEntity<T>> headerEntityArrayList) {
        mItems = new ArrayList<>();
        mHeaderEntities = new IndexHeaderEntity[headerEntityArrayList.size()];

        mItems.addAll(items);
        for (int i = 0; i < headerEntityArrayList.size(); i++) {
            mHeaderEntities[i] = headerEntityArrayList.get(i);
        }

        IndexHeaderEntity [] headerEntities = mHeaderEntities;
